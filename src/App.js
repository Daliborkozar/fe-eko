  ...
  return (
      <Routes>
        <Route path="/*" element={<Layout />}>
          ...
          <Route element={<SuperAdminPrivateRoute token={auth?.token} requiredRole={auth?.user.role} />}>
            <Route path="admintable" element={<AdminList />} />
            <Route path="alluserstable" element={<AllUsersList />} />
          </Route>
          <Route element={<AdminPrivateRoute token={auth?.token} requiredRole={auth?.user?.role} />}>
            <Route path=":orgname/users" element={<UsersInOrganization />} />
          </Route>
          <Route element={<UsersPrivateRoute token={auth?.token} requiredRole={auth?.user?.role} />}>
